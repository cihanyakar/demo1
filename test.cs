...
namespace CodeAnalyze.Business
{
    ...
    {
        public static async Task<List<RemoteFile>> readAllRequest(dynamic payload, CancellationToken cancellationToken)
        {
            ...
            using (var ht = new HttpClient())
            {
                ...
                foreach (var file in files)
                {
                    ...
                    HttpResponseMessage contentResponse = await ht.GetAsync(file.raw_url, cancellationToken);
                    var content = await contentResponse.Content.ReadAsStringAsync();
                    result.Add(new RemoteFile(file.sha as string, file.filename as string, content));
                }
            }
            return result;
        }
